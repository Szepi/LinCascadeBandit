...
As being assumed before, the items in the ground set are sorted in decreasing order of their attraction probabilities, $\bar{w}(1) \geq \ldots \geq \bar{w}(L)$. In this setting, for $K < e \leq L$, $\Delta_{e,K} \geq \Delta_{K+1,K}$. The worst case of expected $n$-step regret is bounded by:
\begin{align*}
...
As we mentioned in former section, we computed $\hat{\theta}_t$ as follow:
...
\end{align*}
Where $X^s_{1 : K \wedge C_s}$ is the matrix whose rows are
...
\end{align*}
Where $ \rnd{\eta}_s $ follows a two-valued distribution that
...
In this way, we can modify the expression of $Y_s$ :
\begin{align*}
...
Under all these notifications and expressions above, we can exactly follow the steps used in the proof
of theorem 2 in Yasin's paper. From there we can get the following expression:
\begin{align*}
  \abs{x^\top \hat{\rnd{\theta}}_t - x^\top \theta_*}
  \leq \norm{x}_{\bar{V}_t^{-1}} (\sum_{s=1}^{t-1}{\norm{X_s^\top \rnd{\eta}_s}_{\bar{V}_t^{-1}}} + \lambda^{0.5} \norm{\theta_*}_2) \,.
\end{align*}

\emph{The mission now is to derive a bound of $\sum_{s=1}^{t-1}{\norm{X_s^\top \rnd{\eta}_s}_{\bar{V}_t^{-1}}}$
following the proof of theorem 1 in Yasin's paper.}

By theorem 1 in Yasin's paper, with $V_s = \lambda I + \sum_{i=1}^{t-1} X_i^\top X_i-X_s^{\top}X_s \,,$
for any $\delta > 0$, with probability at least $1 - \delta$,
 \begin{align*}
  \forall t \geq 0, \sum_{s=1}^{t-1}{\norm{X_s^\top \rnd{\eta}_s}_{\bar{V}_t^{-1}}}
  \leq R \sum_{s=1}^{t-1} \sqrt{2 \log\left(\frac{\mathrm{det}(\bar{V}_t)^{1/2} \mathrm{det}(\bar{V}_t-X_s^{\top}X_s)^{-1/2} }{\delta}\right)}\,
  = R \sum_{s=1}^{t-1} \sqrt{\log\left(\frac{\mathrm{det}(\bar{V}_t)}{\delta \mathrm{det}(\bar{V}_t-X_s^{\top}X_s)}\right)}\,.
\end{align*}

Than, by following the steps in Yasin's paper, we can get the bound from this point:
\begin{align*}
  \norm{\hat{\rnd{\theta}}_t - \theta_*}_{\bar{V}_t}
  \leq R \sum_{s=1}^{t-1} \sqrt{\log\left(\frac{\mathrm{det}(\bar{V}_t)}{\delta \mathrm{det}(\bar{V}_t-X_s^{\top}X_s)}\right)} + \lambda^{1/2}S \,.
\end{align*}
